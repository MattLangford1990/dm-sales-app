...
async def get_image_manifest():
    """
    Get manifest of all product images from local image_urls.json.
    Returns SKU -> list of available image suffixes.
    Uses pre-loaded data from startup - no external API calls.
    """
    # Build manifest from _image_urls (loaded at startup from image_urls.json)
    # _image_urls format: {"SKU": "https://cdn.appdmbrands.com/..."}
    # We just need to return SKUs that have images
    manifest = {}
    for sku in _image_urls.keys():
        manifest[sku] = [""]  # Main image only, suffix is empty string
    
    return {
        "manifest": manifest,
        "total_skus": len(manifest),
        "skus_with_multiple": 0,
        "source": "local_image_urls_json",
        "from_cache": True
    }
...
async def admin_refresh_image_manifest(agent: TokenData = Depends(require_admin)):
    """Refresh image manifest - now just returns current local data"""
    result = await get_image_manifest()
    return {
        "message": "Image manifest loaded from local file",
...
# ============ Trade Show / Catalogue Request Capture ============

from database import CatalogueRequest as CatalogueRequestModel
...
async def trade_show_capture(data: TradeShowCaptureRequest):
    ...
    Saves lead data to database for admin review.
    No authentication required.
    """
    try:
        db = SessionLocal()
        try:
            # Create new catalogue request
            request_id = str(uuid.uuid4())
            new_request = CatalogueRequestModel(
                id=request_id,
                ...
                catalogue_format=data.catalogueFormat,
                brands=data.brands,
                notes=data.notes or "",
                is_read=False
            )
            db.add(new_request)
            db.commit()
            print(f"CATALOGUE REQUEST: Saved {data.businessName} - {data.email}")
        finally:
            db.close()
...
@app.get("/api/admin/catalogue-requests")
async def admin_get_catalogue_requests(
    agent: TokenData = Depends(require_admin)
):
    """
    Get all catalogue requests (admin only).
    Returns requests sorted by date, newest first.
    """
    db = SessionLocal()
    try:
        requests = db.query(CatalogueRequestModel).order_by(
            CatalogueRequestModel.created_at.desc()
        ).all()
        ...
        return {
            "requests": [
                {
                    ...
                    "catalogue_format": req.catalogue_format,
                    "brands": req.brands,
                    "notes": req.notes,
                    "is_read": req.is_read,
            ...
            "total": len(requests),
            "unread": len([r for r in requests if not r.is_read])
        }
    finally:
        db.close()


@app.get("/api/admin/catalogue-requests/unread-count")
async def admin_get_unread_count(
    agent: TokenData = Depends(require_admin)
):
    """
    Get count of unread catalogue requests (admin only).
    Used for notification badge.
    """
    db = SessionLocal()
    try:
        count = db.query(CatalogueRequestModel).filter(
            CatalogueRequestModel.is_read == False
        ).count()
...
@app.put("/api/admin/catalogue-requests/{request_id}/read")
async def admin_mark_request_read(
    request_id: str,
    agent: TokenData = Depends(require_admin)
):
    """
    Mark a catalogue request as read (admin only).
    """
    db = SessionLocal()
    try:
        req = db.query(CatalogueRequestModel).filter(
            CatalogueRequestModel.id == request_id
        ).first()
        if not req:
...
@app.put("/api/admin/catalogue-requests/mark-all-read")
async def admin_mark_all_read(
    agent: TokenData = Depends(require_admin)
):
    """
    Mark all catalogue requests as read (admin only).
    """
    db = SessionLocal()
    try:
        db.query(CatalogueRequestModel).filter(
            CatalogueRequestModel.is_read == False
        ).update({"is_read": True})
        db.commit()
        return {"success": True}
...
@app.delete("/api/admin/catalogue-requests/{request_id}")
async def admin_delete_request(
    request_id: str,
    agent: TokenData = Depends(require_admin)
):
    """
    Delete a catalogue request (admin only).
    """
    db = SessionLocal()
    try:
        req = db.query(CatalogueRequestModel).filter(
